          ...
          return (
            <div key={p.id} className="card" style={{ cursor: 'pointer', padding: 0, overflow: 'hidden' }}
              ...
              <div style={{ background: 'var(--bg)', height: 160, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
                {p.imagine
                  ? <img src={p.imagine} alt={p.name} style={{ maxHeight: 145, maxWidth: '90%', objectFit: 'contain' }} />
                ...
                {p.stoc === 0 && (
                  <div style={{ position: 'absolute', inset: 0, background: 'rgba(255,255,255,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--red-text)', background: 'var(--red-bg)', padding: '4px 10px', borderRadius: 20 }}>Stoc epuizat</span>
                  </div>
                )}
              </div>
              {/* Info */}
              <div style={{ padding: '10px 12px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 500, color: 'var(--text)' }}>{lei(pricing.pretUnitar)}<span style={{ fontSize: 11, color: 'var(--text3)', fontWeight: 400 }}>/{p.unitate}</span></div>
        ...
        <div className="modal-overlay" onClick={() => setSelected(null)}>
          <div className="modal" style={{ width: 580 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: 20, marginBottom: 20 }}>
              <div style={{ background: 'var(--bg)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 12 }}>
                {selected.imagine
                  ? <img src={selected.imagine} alt={selected.name} style={{ maxWidth: '100%', maxHeight: 180, objectFit: 'contain' }} />
              ...
              <div>
                ...
                {selected.specs && (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 12px', fontSize: 12 }}>
                    {selected.specs.ply && <div><span style={{ color: 'var(--text3)' }}>Straturi:</span> {selected.specs.ply}</div>}
                    {selected.specs.sheets && <div><span style={{ color: 'var(--text3)' }}>Foi:</span> {selected.specs.sheets}</div>}
                    {selected.specs.weight && <div><span style={{ color: 'var(--text3)' }}>Greutate:</span> {selected.specs.weight}</div>}
                    {selected.specs.pack && <div><span style={{ color: 'var(--text3)' }}>Pachet:</span> {selected.specs.pack}</div>}
                    {selected.specs.packsPerBox && <div><span style={{ color: 'var(--text3)' }}>Role/bax:</span> {selected.specs.packsPerBox}</div>}
                    <div>
                      <span style={{ color: 'var(--text3)' }}>Stoc:</span>{' '}
                      {selected.stoc > 1000 ? <span style={{ color: 'var(--green)' }}>Disponibil</span>
                        : selected.stoc > 0 ? <span style={{ color: 'var(--orange-text)' }}>Limitat ({selected.stoc})</span>
            ...
            <table>
              ...
              <tbody>
                {selected.tierPricing.map((t, i) => {
                  ...
                  return (
                    <tr key={i}>
                      <td style={{ fontSize: 12 }}>{t.cantMin}{t.cantMax >= 9999 ? '+' : ` – ${t.cantMax}`} role</td>
                      <td>{lei(t.pret)}</td>
