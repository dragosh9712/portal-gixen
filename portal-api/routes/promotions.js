...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    res.json(result.recordset.map(r => ({
      ...
      conditions_json: undefined, actions_json: undefined,
    })))
  ...
  try {
    ...
    await query(`
      INSERT INTO promotion_rules (
        ...
        valid_from, valid_until, customer_group, brand,
        conditions_json, actions_json, cumulative
      ) VALUES (
        @id, @name, @desc, @type, @pri, @active,
        @from, @until, @cgroup, @brand,
        @cond, @act, @cumul
      )`, {
      id, name: r.name || 'Regulă nouă', desc: r.description || '',
      type:   r.rule_type    || 'LINE_DISCOUNT',
      pri:    r.priority     || 10,
