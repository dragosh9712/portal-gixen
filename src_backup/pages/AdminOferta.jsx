...
import { calculeazaCos, toRole, getTierPret, pretPerUnitate } from '../promoEngine.js'
...
export default function AdminOferta() {
  ...
  // Run promo engine on ofertă lines
  const liniiMotorOferta = liniiCalc.map(l => ({
    productId: l.product.id,
    cantitate: l.cantitate,
    cantRole: l.cantitate, // in ofertă lucrăm în role
    unitateSel: 'rolă',
    pretBazaPerRola: l.product.pretBaza,
    totalBrutLinie: l.pretBaza * l.cantitate,
    produs: l.product,
  }))

  // Use null firm for offer (no client selected yet) or find firm if firmId set
  const ofertaFirma = null

  const { discountLinii: promoDiscLinii, totalBrut: subBaza, totalDiscount: promoTotalDisc, totalNet: promoTotalNet } =
    calculeazaCos(liniiMotorOferta, ofertaFirma, db)

  const subBaza2 = liniiCalc.reduce((s, l) => s + l.pretBaza * l.cantitate, 0)
  // Combine manual discounts (discount extra per line) with promo engine discounts
  const manualDisc = liniiCalc.reduce((s, l) => s + (l.pretBaza - l.pretFinal) * l.cantitate, 0)
  const totalDisc = manualDisc
  const totalNet = subBaza2 - totalDisc
  const tvaVal = totalNet * TVA
  const totalCuTva = totalNet + tvaVal

  // Build discount lines for display: manual line discounts + promo engine
  const toateDiscountLinii = [
    ...liniiCalc.map((l, i) => l.discountProcent > 0 ? {
      refLinieIdx: i,
  ...
  // Recalc total with all discounts
  ...
  <table>
    ...
    <tbody>
      ${liniiCalc.map((l, i) => {
        ...
        const specs = l.product.specs ? `${l.product.specs.ply?l.product.specs.ply+' str. ':''}${l.product.specs.sheets?l.product.specs.sheets+' foi ':''}${l.product.specs.pack||''}`.trim() : ''
        return `<tr>
          <td class="center" style="color:#94a3b8;font-size:7.5pt">${i+1}</td>
          <td class="center" style="padding:2pt">${l.product.imagine?`<img src="${l.product.imagine}" style="width:24px;height:24px;object-fit:contain" onerror="this.style.display='none'">`:''}</td>
          ...
          <td class="center">${l.discountProcent>0?`<span class="disc-badge">-${l.discountProcent.toFixed(1)}%</span>`:'—'}</td>
    ...
    <tbody>
      <tr class="tot-row tot-sep"><td colspan="8"></td><td class="right" style="color:#475569">Subtotal brut:</td><td class="right" style="color:#475569">${fmtRon(subBaza)}</td><td class="right" style="color:#475569">${fmtRon(subBaza*(1+TVA))}</td></tr>
      ${totalDisc>0?`<tr class="tot-row"><td colspan="8"></td><td class="right" style="color:#16a34a;font-weight:bold">Total reduceri:</td><td class="right" style="color:#16a34a;font-weight:bold">-${fmtRon(totalDisc)}</td><td class="right" style="color:#16a34a;font-weight:bold">-${fmtRon(totalDisc*(1+TVA))}</td></tr>`:''}
      <tr class="tot-row"><td colspan="8"></td><td class="right" style="font-weight:bold">Total net:</td><td class="right" style="font-weight:bold">${fmtRon(totalNet)}</td><td class="right" style="font-weight:bold">${fmtRon(totalCuTva)}</td></tr>
      <tr class="tot-row"><td colspan="8"></td><td class="right" style="color:#94a3b8">TVA ${Math.round(TVA*100)}%:</td><td class="right" style="color:#94a3b8">${fmtRon(tvaVal)}</td><td class="right" style="color:#94a3b8">—</td></tr>
      <tr class="grand-row"><td colspan="8"></td><td class="right">TOTAL DE PLATĂ:</td><td class="right">${fmtRon(totalNet)}<br><span style="font-size:6.5pt;font-weight:normal;opacity:.8">fără TVA</span></td><td class="right">${fmtRon(totalCuTva)}<br><span style="font-size:6.5pt;font-weight:normal;opacity:.8">cu TVA</span></td></tr>
    </tbody>
  </table>
