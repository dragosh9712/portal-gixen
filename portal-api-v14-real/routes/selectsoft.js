...
async function ensureCodPartenColumn() {
  await query(`
...
// Match după CUI (cui ↔ cod_fiscal, ignorând prefixul RO).
router.post('/sync-customers', authenticateToken, requireAdmin, async (req, res) => {
  try {
    await ensureCodPartenColumn()
    let offset = 0
    const limit = 100
    ...
    const customersResult = await query('SELECT id, name, cui FROM customers WHERE cui IS NOT NULL')
    const byCui = new Map()
    for (const c of customersResult.recordset) {
      const k = normalize(c.cui)
      if (k) byCui.set(k, c)
    }
...
router.post('/import-history', authenticateToken, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const custRes = await query('SELECT id, cui, selectsoft_cod_parten FROM customers')
        ...
        for (const p of parteneri) {
          try {
            ...
            await query(`
              ...
              INSERT INTO customers (id, name, cui, reg_com, phone, email,
                address, status, selectsoft_cod_parten, created_at)
              VALUES (@id, @name, @cui, @regCom, @phone, @email,
                @address, 'inactiv', @codParten, GETDATE())`, {
              id, name: p.denumire, cui: p.cod_fiscal || '',
              regCom: p.numar_registru_comert || '', phone: p.telefon || '', email: p.email || '',
              address: p.adresa || p.strada || '',
              codParten,
            })
