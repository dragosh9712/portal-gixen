...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const uRes = await query(
      `SELECT u.email, u.name, c.name AS firm_name
       FROM users u JOIN customers c ON u.customer_id = c.id
       WHERE u.customer_id = @cid AND u.role = 'client'
       ORDER BY u.created_at ASC`, { cid: o.customer_id }
    ).catch(() => ({ recordset: [] }))
    const u = uRes.recordset[0]
    if (u) {
      email.sendOrderPlaced({
        email: u.email, name: u.name, firmName: u.firm_name,
        order: {
          nr, id,
          paymentType: o.payment_type, transportType: o.transport_type,
          netTotal: Math.round(netTotal * 100) / 100, tvaTotal, grossTotal,
          lines: (o.lines || []).map(l => ({
            productName: l.product_name || l.productId || l.product_id,
            uomCode: l.unitateSel || l.uom_code,
            cantitate: l.cantitate || l.quantity,
...
router.put('/:id/status', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    // Email notificare schimbare status
    const oRes = await query(
      `SELECT o.order_number, o.gross_total, o.invoice_number, o.delivery_date, o.customer_id,
              u.email, u.name, c.name AS firm_name
       FROM orders o
       JOIN customers c ON o.customer_id = c.id
       JOIN users u ON u.customer_id = c.id AND u.role = 'client'
       WHERE o.id = @id
       ORDER BY u.created_at ASC`, { id: req.params.id }
    ).catch(() => ({ recordset: [] }))
    const row = oRes.recordset[0]
    if (row) {
      email.sendOrderStatusChanged({
        email: row.email, name: row.name, firmName: row.firm_name,
        order: { nr: row.order_number, grossTotal: row.gross_total, nrFactura: row.invoice_number, dataLivrare: row.delivery_date },
        newStatus: status,
      }).catch(() => {})
    }
  } catch (err) {
